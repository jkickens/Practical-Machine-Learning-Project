...
pmltest <- read.csv("pml-test.csv", header=TRUE)
```
...
```{r}
pmltrain4 <- pmltrain3[, which(as.numeric(colSums(is.na(pmltrain3))) != 14410)]
```
...
pmltest4 <- pmltest3[,which(as.numeric(colSums(is.na(pmltest3))) != 4806)]
```
...
mytrain <- pmltest4[inTrain,]
mytest <- pmltest4[-inTrain,]
```
...
modFit <- train(mytrain1$classe ~ ., method="rf", data=trainPC)
```
...
```{r}
testPC <- predict(preProc, mytest1[,-54])
pred <- predict(modFit, testPC)
confusionMatrix(mytest1$classe, pred)
```
